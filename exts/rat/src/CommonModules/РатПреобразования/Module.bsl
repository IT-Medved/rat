...
Функция СериализоватьЗначение(Значение) Экспорт
	...
	ИначеЕсли ТипЗначения = Тип("РасписаниеРегламентногоЗадания") Тогда
		
		Результат = СериализоватьЗначение(
			РатРегламентныеЗадания.ОписаниеРасписания(Значение));
		
	ИначеЕсли ТипЗначения = Тип("СтандартнаяДатаНачала") Тогда
		
		Результат = Новый Структура("Вариант, Дата");
		Результат.Вариант = СериализоватьЗначение(Значение.Вариант);
		Результат.Дата = СериализоватьЗначение(Значение.Дата);
		
	ИначеЕсли ТипЗначения = Тип("СтандартныйПериод") Тогда
		
		Результат = Новый Структура("Вариант, ДатаНачала, ДатаОкончания");
		Результат.Вариант = СериализоватьЗначение(Значение.Вариант);
		Результат.ДатаНачала = СериализоватьЗначение(Значение.ДатаНачала);
		Результат.ДатаОкончания = СериализоватьЗначение(Значение.ДатаОкончания);
		
	ИначеЕсли ТипЗначения = Тип("НастройкиКомпоновкиДанных") Тогда
		
		Результат = Новый Структура("type, value", РатТипыДанных.ПредставлениеТипа(ТипЗначения), ЗначениеВСтрокуВнутр(Значение));

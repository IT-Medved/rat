...
//       Параметры);
//
Функция ВычислитьЗначениеМетода(ИмяМодуля, ИмяМетода, Параметры, БезопасныйРежим = Истина) Экспорт
	
	МетодПереопределения = МетодПереопределения(ИмяМодуля, ИмяМетода);
	Если МетодПереопределения <> Неопределено Тогда
		Возврат РатАлгоритмы.ВызватьФункцию(МетодПереопределения, Параметры, БезопасныйРежим);
	КонецЕсли;

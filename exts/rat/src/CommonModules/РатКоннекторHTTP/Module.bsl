...
Функция ВыполнитьЗапрос(ОписаниеЗапроса) Экспорт
	...
	Попытка
		...
		ДанныеОтвета.Тело = ПолучитьТелоКакСтроку(Ответ, КодировкаОтвета);
...
Функция Запрос(ОписаниеЗапроса)
	...
	Если ЗначениеЗаполнено(ОписаниеЗапроса.Тело) Тогда
		...
		Иначе
			...
			Если НЕ КодировкаЭтоUTF8(КодировкаЗапроса) Тогда
...
Функция КодировкаЭтоUTF8(Кодировка)
	...
	Если ТипЗнч(Кодировка) = Тип("Строка") Тогда
		НормализованнаяКодировка = НРег(СтрЗаменить(Кодировка, " ", ""));
		Возврат НормализованнаяКодировка = "UTF-8" Или НормализованнаяКодировка = "UTF8";
	КонецЕсли;
	
	Возврат Кодировка = КодировкаТекста.UTF8;
	
КонецФункции

Функция СтрокаКодировки(Кодировка)
...
Функция ПолучитьТелоКакСтроку(Ответ, Кодировка)
	
	Попытка
		Если ЗначениеЗаполнено(Кодировка) Тогда
			Возврат Ответ.ПолучитьТелоКакСтроку(Кодировка);
		КонецЕсли;
	Исключение
		// Fallback for platforms without encoding parameter.
	КонецПопытки;
...
Функция ОпределитьКодировкуОтвета(Ответ, ОписаниеЗапроса)
	...
	ЗначениеContentType = ПолучитьЗначениеЗаголовка(Ответ.Заголовки, "Content-Type");
	Если ЗначениеЗаполнено(ЗначениеContentType) И Найти(НРег(ЗначениеContentType), "application/json") > 0 Тогда
		Возврат КодировкаТекста.UTF8;
	КонецЕсли;
...
Функция ОпределитьКодировкуИзЗаголовков(Заголовки, КодировкаПоУмолчанию)
	...
	ЗначениеContentType = ПолучитьЗначениеЗаголовка(Заголовки, "Content-Type");
	...
	Для Каждого Часть Из Части Цикл
		...
		Позиция = Найти(НРег(Часть), "charset=");
...
Функция ПолучитьЗначениеЗаголовка(Заголовки, Знач ИмяЗаголовка)
